...
import PublicoIgreja from 'src/utils/publicoIgreja';
import GerarPdf from './pdfs/pdf';

const janela = TamanhoJanela();
let altura2;
let largura2;
if (janela.height < 500) altura2 = 500;
else altura2 = janela.height;
// const validateDate = require('validate-date');
if (janela.width < 300) largura2 = 300;
else largura2 = janela.width;
if (janela.width > 410) largura2 = 410;
else largura2 = janela.width;
const Container = styled.div`
  ...
  .rccs__card--hipercard .rccs__card__background {
    background: linear-gradient(25deg, #424242, #e0511a);
  }
...
const useStyles = makeStyles((theme) => ({
  ...
  desktopDrawer: {
    width: largura2,
    left: janela.width / 2 - largura2 / 2,
    borderRight: 'none',
  },
}));

export default function CheckoutT({ dadosCC }) {
  const { email } = dadosCC;
  const { cpf } = dadosCC;
  const { nome } = dadosCC;
  const { qtyA } = dadosCC;
  ...
  const { total } = dadosCC;
  const { fone } = dadosCC;
  const { estadia } = dadosCC;
  const { genero } = dadosCC;
  const { grau } = dadosCC;
  const { horario } = dadosCC;
  const { igrejas } = dadosCC;
  const { jEstadual } = dadosCC;
  const { nascimento } = dadosCC;
  const { dataChegada } = dadosCC;
  const { Responsavel } = dadosCC;
  const { Secretaria } = dadosCC;
  const { qtyC1 } = dadosCC;
  const { qtyC2 } = dadosCC;
  const { fpag } = dadosCC;
  const { transporte } = dadosCC;
  const { Evento } = dadosCC;
  const { Jurisdicao } = dadosCC;
  const { Distrito } = dadosCC;
  // const { logoEvento } = dadosCC;
  const { jurisdicaoEvento } = dadosCC;

  let publico;
  if (jurisdicaoEvento === '0') publico = PublicoIgreja();
  ...
  const [anoF, setAnoF] = React.useState('');
  ...
  const mp = useMercadopago.v2(publico, {
    locale: 'pt-BR',
  });
  ...
  const erros = [
    ...
    {
      erro: '208',
      mensagem: 'Digite o mês de vencimento',
    },
    {
      erro: '209',
      mensagem: 'Digite o ano de vencimento',
    },
    {
      erro: '212',
      mensagem: 'Informe seu documento',
    },
    {
      erro: '213',
      mensagem: 'Informe seu documento',
    ...
    {
      erro: '220',
      mensagem: 'Informe seu banco emissor',
    },
    {
      erro: '221',
      mensagem: 'Digite o nome e sobrenome',
    },
    {
      erro: '224',
    ...
    {
      erro: 'E302',
      mensagem: 'Confira o código de segurança',
    },
    {
      erro: '316',
    ...
    {
      erro: '322',
      mensagem: 'Confira seu documento',
    },
    {
      erro: '323',
      mensagem: 'Confira seu documento.',
  ...
  //= ===================================================================
  // 1. paymentMethods
  async function paymentMethods(bin) {
    const paymentMethods2 = await mp.getPaymentMethods({ bin });
    const juros = paymentMethods2.results[0].payer_costs;

    const descricao = [];

    for (let i = 0; i < paymentMethods2.results[0].payer_costs.length; i += 1) {
      //  parcelas[i]. = juros[i];
      const valInstallment = Number(
        Number(
          Number(amount) / juros[i].installments +
            ((Number(amount) / juros[i].installments) *
              Number(juros[i].installment_rate)) /
              100,
        ).toFixed(2),
      ).toFixed(2);
      const vTotal = Number(valInstallment * juros[i].installments).toFixed(2);
      if (juros[i].installments === 1) descricao[i] = 'parcela';
  ...
  const handleFocus = (e) => {
    ...
    if (e.target.dataset.name === 'securityCode') {
      setData({
        ...data,
        focus: 'cvc',
      });
  ...
  const handleEnter = (event) => {
    if (event.key.toLowerCase() === 'enter') {
      ...
      if (event.target.id === 'form-checkout__identificationNumber')
  ...
  const handleSubmit = async () => {
    ...
    if (validarDoc) {
      ...
      if (conexao.status && conexao.status !== 'active') {
      ...
      if (!checagem.status || checagem.status !== 'active') {
        if (checagem.status) {
          ...
          setValorErro(` ${checagem}`);
          setOpenDrawer(true);
        }
      } else {
        let url = '';
        url = '/api/mercadoPagoCartao';
        api
          .post(url, {
            token: checagem.id,
          ...
          .then((response) => {
            ...
            if (
              ...
              if (response.data.body.status === 'in_process') {
                setValorErro(
                  'Estamos processando o pagamento. Aguarde 6 horas, pode ser que por motivo de segurança sua compra pode ser Recusada.',
                );
              ...
              if (response.data.body.status_detail === 'accredited') {
                setValorErro(
                  'Pronto, seu pagamento foi aprovado! Para conferir seu Ticket é só FECHAR, clicar em MINHA INSCRIÇÃO e digitar seu CPF.',
                );
      ...
      <form id="form-checkout">
        <Box>
                ...
                <Box mt={-1}>
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      if (CardNumber.length === 6) paymentMethods(CardNumber);
                ...
                <Box mt={-1}>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const nomeSemAcento = novoNome
                        .normalize('NFD')
                        .replace(/([\u0300-\u036f]|[^0-9a-zA-Z .])/g, '');
                      setName(nomeSemAcento.toUpperCase());
                    }}
                  />
                </Box>
              </Grid>
            ...
            <Box display="flex" width="100%" mt={2} ml={1}>
              <Grid item xs={12} md={12}>
                <Typography
                  ...
                  gutterBottom
                >
                  Número de Parcelas (juros pode alterar dependendo do Banco)
                </Typography>
              </Grid>
            </Box>
          </Box>
          ...
          >
            ...
            >
              {!carregar ? (
                <Box color="white">FAZER O PAGAMENTO</Box>
              ) : (
                <Box display="flex">
