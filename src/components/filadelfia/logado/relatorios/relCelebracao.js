...
function createPontuacao(
  ...
  Crescimento, // apenas para avaliacao de crescimento
) {
  return {
    Relatorio, // vale 1 ponto
    Pontualidade, // vale 1 ponto
...
function RelatorioCelebracao({ rolMembros, perfilUser, visitantes }) {
  ...
  const criarPontuacao = () => {
    ...
    const pontosPresentes = 0;
    const pontosVisitantes = 0;
    const pontosEventos = 0;
    const pontosNovoMembro = 0;
    const pontosVisitas = 0;
    ...
    const pontosCrescimento = parseFloat(
      (100 * (TotalPercentual - pontosTotalAnterior)) / pontosTotalAnterior,
    ).toFixed(2);
