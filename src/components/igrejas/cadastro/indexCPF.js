...
function Cadastro({ rolMembros, perfilCPF }) {
  ...
  const cadastrarEmailMembro = () => {
    if (usuarioMembro.length > 0) {
      ...
      api
        .post(`/api/igrejas/cadastrarCPFMembro`, {
          CPF: perfilCPF,
    ...
    if (nome !== '') {
      ...
      if (!vaiNome) {
        ...
        if (checarNome.length === 1) {
          ...
          if (
            moment(checarNome[0].Nascimento.substring(0, 10)).format(
              'DD/MM/YYYY',
            ) === nascimento
          ) {
