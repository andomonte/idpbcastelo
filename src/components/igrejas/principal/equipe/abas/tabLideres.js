...
export default function TabCelula({
  ...
  let listaParcial1 = lideranca.filter(
    (val) =>
  ...
  if (perfilUser.Funcao === 'Supervisor')
    listaParcial1 = lideranca.filter(
      (val) =>
  ...
  if (Funcao === 'PastorDistrito') {
    lideresSetor = listaParcial.sort((a, b) => {
      if (new Date(a.Distrito) > new Date(b.Distrito)) return 1;
      if (new Date(b.Distrito) > new Date(a.Distrito)) return -1;
      return 0;
    });
  }
  if (Funcao === 'Professor') {
    lideresSetor = listaParcial.sort((a, b) => {
      if (new Date(a.Nome) > new Date(b.Nome)) return 1;
      if (new Date(b.Nome) > new Date(a.Nome)) return -1;
      return 0;
    });
  }
  return (
                    ...
                    <Box ml={0.2} display="flex">
                      <ThemeProvider theme={theme}>
                        <Typography variant="h4">
                          {Coordenacao.Coordenacao_Nome.length > 18
                            ? Coordenacao.Coordenacao_Nome.substring(
                                0,
                                Coordenacao.Coordenacao_Nome.lastIndexOf(' '),
                              ).toUpperCase()
                            : Coordenacao.Coordenacao_Nome.toUpperCase()}
                        </Typography>
                      </ThemeProvider>
