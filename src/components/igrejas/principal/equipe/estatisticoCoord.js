...
function Celula({
  ...
  const distritoNome = distritos.filter(
    (val) =>
      Number(val.Distrito) === Number(coordenacoes[contNumeroCoord].Distrito) &&
      val.Status,
  );
  ...
  React.useEffect(() => {
    if (rolMembros) {
      const rolCoord = rolMembros.filter(
        (val) =>
          Number(val.Coordenacao) ===
            Number(coordenacoes[contNumeroCoord].Coordenacao) &&
          Number(val.Distrito) ===
            Number(coordenacoes[contNumeroCoord].Distrito),
      );
      ...
      if (contNumeroCoord) {
        ...
        setNumeroCoord(liderancaF.length);
  ...
  return (
    <Box width="100%" fontSize="16px" sx={{ fontFamily: 'Rubik' }}>
      ...
      >
        ...
        >
          <ThemeProvider theme={theme}>
            <Typography variant="h4"> COORDENADORES</Typography>
          </ThemeProvider>
        </Box>
        <Box
          color="white"
