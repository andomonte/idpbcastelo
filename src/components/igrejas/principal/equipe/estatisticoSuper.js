...
function Celula({
  ...
  const distritoNome = distritos.filter(
    (val) =>
      Number(val.Distrito) === Number(supervisoes[contNumeroSuper].Distrito) &&
      val.Status,
  );
  const coordNome = coordenacoes.filter(
    (val) =>
      ...
      Number(val.Coordenacao) ===
        Number(supervisoes[contNumeroSuper].Coordenacao),
  );
      ...
      const rolCoord = rolMembros.filter(
        (val) =>
          Number(val.Coordenacao) ===
            Number(supervisoes[contNumeroSuper].Coordenacao) &&
          Number(val.Distrito) ===
            Number(supervisoes[contNumeroSuper].Distrito) &&
          Number(val.Supervisao) ===
            Number(supervisoes[contNumeroSuper].Supervisao),
      );
  ...
  return (
    <Box width="100%" fontSize="16px" sx={{ fontFamily: 'Rubik' }}>
      ...
      >
        ...
        >
          <ThemeProvider theme={theme}>
            <Typography variant="h4"> SUPERVISORES</Typography>
          </ThemeProvider>
        </Box>
      </Box>
      <Box
